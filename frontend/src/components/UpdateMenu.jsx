...
const UpdateMenuForm = () => {
  ...
  const fetchMenuByName = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/food/menu/getByName/${menuName}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(`http://localhost:8000/food/menu/${formData._id}`, formData);
  ...
  return (
    <div className="flex items-center justify-center w-full">
      <div className="bg-neutral-800/30 backdrop-blur-md p-8 rounded-lg shadow-lg w-full max-w-md">
        <h1 className="text-2xl font-bold mb-6 text-center text-white">Update Menu</h1>

        {/* Search Menu By Name */}
        <div className="mb-4">
          <label className="p-1 block font-bold text-neutral-100">Enter Menu Name:</label>
          <input
            type="text"
            value={menuName}
        ...
        {isMenuFound && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="p-1 block font-bold text-neutral-100">Short Description:</label>
              <input
                type="text"
                name="short_desc"
            ...
            <div>
              <label className="p-1 block font-bold text-neutral-100">Description:</label>
              <textarea
                name="description"
            ...
            <div>
              <label className="p-1 block font-bold text-neutral-100">Ratings (0-5):</label>
              <input
                type="number"
                name="ratings"
            ...
            <button type="submit" className="w-full py-2 bg-orange-700 font-bold text-white rounded-lg hover:bg-orange-600">
              Update Menu
            </button>
          </form>
        )}
