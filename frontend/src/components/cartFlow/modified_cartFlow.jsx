  ...
  useEffect(() => {
	const fetchAddresses = async () => {
	  try {
		...
		const response = await axios.get("http://localhost:8000/data/userAddress", {
		  headers: { Authorization: `Bearer ${token}` },
		});
  
		if (response.data.data.address) {  // Since address is an object
		  ...
		  toast.error("No address found", { position: "bottom-right" });
		}
	  } catch (error) {
		toast.error(
 ...
 /*  const deliveryFee = state.cart?.deliveryFee || 2.99;
  ...
  const renderMobileHeader = () => (
    <div className="md:hidden flex items-center justify-between mb-4">
      <button onClick={() => step > 1 && setStep(step - 1)} className="p-2">
        ...
        <ul className="space-y-4">
          {state.cart?.items?.map((item) => (
            <CartItems
              ...
              isProcessed={state.cart?.status?.toLowerCase() === "processed"}
            />
          ))}
        </ul>
      ...
      <div className="space-y-4">
        <div>
          ...
          <select
            ...
            onChange={(e) => setSelectedAddress(addresses[e.target.value])} // Get object by index
          >
            {addresses.length > 0 ? (
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3">
                ...
                <>
                  ...
                  <p className="text-sm text-gray-600">{selectedAddress.state} - {selectedAddress.zipCode}</p>
                </>
              ) : (
