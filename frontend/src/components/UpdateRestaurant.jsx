...
const UpdateRestaurantForm = () => {
  ...
  const fetchRestaurantByName = async () => {
    try {
      ...
      toast.error("Error fetching restaurant. Please check the name and try again.");
      setIsRestaurantFound(false);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="bg-red-800/30 backdrop-blur p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        {isRestaurantFound && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Location */}
            <div>
              <label className="p-1 block font-bold text-neutral-100">Location:</label>
              <input
                type="text"
                name="location"
            ...
            {/* Contact */}
            <div>
              <label className="p-1 block font-bold text-neutral-100">Contact:</label>
              <input
                type="text"
                name="contact"
            ...
            {/* Image Upload */}
            <div>
              <label className="p-1 block font-bold text-neutral-100">Images:</label>
              <button
                type="button"
            ...
            {/* Operating Hours */}
            <div>
              <h3 className="p-1 block font-bold text-neutral-100">Operating Hours</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(formData.operatingHours).map(([day, hours]) => (
                  <div key={day}>
                    <label className="block text-sm font-medium text-gray-100 capitalize">{day}</label>
                    <input
                      type="text"
                      value={hours}
