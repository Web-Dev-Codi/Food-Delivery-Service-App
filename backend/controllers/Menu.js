...
export const addMenu = async (req, res) => {
    try {
        const { name, description, price, category, imageUrl, availability, restaurant } = req.body;

        // Validate required fields
        if (!name || !description || !price || !category || !imageUrl || !availability || !restaurant) {
        ...
        const newMenu = await FoodItem.create({ name, description, price, category, imageUrl, availability, restaurant });
...
export const updateMenu = async (req, res) => {
    try {
      const id = req.params.id;
      const updatedMenu = await FoodItem.findByIdAndUpdate(id, req.body, { new: true });
        if (!updatedMenu) {
